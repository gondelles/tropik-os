# Tropik OS

Private canonical operating system for **Tropik Goods**.

> 🔒 PRIVATE_CANONICAL — This repository is not a public portfolio repository and must remain private.

## Brand hierarchy

- **Tropik Goods** — umbrella / venture system.
- **Tropik** — current commerce / merch brand.
- **Habit App** — software venture, working name; prior context pending migration.
- **Tropik Academy** — internal working label for education; public brand not locked.

## Current portfolio

| Area | Status | Current focus |
|---|---|---|
| Tropik Venture OS | GREEN | Dual-AI context, governance and handoffs |
| Tropik Commerce | GREEN | WooCommerce audit and controlled relaunch |
| Habit App | YELLOW | Context migration and validation before build |
| Education | YELLOW | Bootcamp asset inventory and one-course opportunity selection |
| Amazon scale | RED | Future channel after product/operations validation |

## Repository map

```text
00_HQ/            Cross-venture canonical context and governance
10_COMMERCE/      Tropik commerce venture
20_HABIT_APP/     Habit software venture
30_EDUCATION/     Education / Udemy venture
80_SHARED/        Reusable templates and Build in Public assets
90_AI_SYSTEM/     AI prompts, handoffs, skills and schemas
```

## Operating rule

**Idea → Assumption → Validation → Decision → Manual Process → SOP → Automation → Scale**

Do not promote AI output into canonical truth without the approval rules defined in `00_HQ/governance/`.

## Public work

Public portfolio artifacts must be created as sanitized derivatives under the publication policy. Never make this canonical repository public in order to showcase the work.
